...
export function parseColdCases(markdown) {
  ...
  parseNarrativeTeasers(markdown, byUrl);
  return [...byUrl.values()]
    .map(enrichCase)
...
function parseCandidateTable(markdown, byUrl) {
  ...
  for (const row of rows) {
    ...
    const link = parseMarkdownLink(columns[1]);
    if (!link) continue;
...
function parseNarrativeTeasers(markdown, byUrl) {
  const teaserMatch = markdown.match(/## Narrative Teasers([\s\S]*)$/);
  if (!teaserMatch) return;

  const blocks = teaserMatch[1].split(/\n###\s+/).map((block) => block.trim()).filter(Boolean);
  const byQuestionId = new Map([...byUrl.values()].map((item) => [item.question_id, item]));

  for (const rawBlock of blocks) {
    const block = rawBlock.startsWith("###") ? rawBlock : `### ${rawBlock}`;
    const header = block.match(/^###\s+(\d+)\s+-\s+(.+)$/m);
    if (!header) continue;
    const url = block.match(/-\s+URL:\s+(.+)/)?.[1]?.trim();
    const teaser = block.match(/-\s+Teaser:\s+([\s\S]*?)(?=\n-\s+\w+:|\n###|$)/)?.[1]?.trim();
    const item = (url && byUrl.get(url)) || byQuestionId.get(header[1]);
    if (item && teaser) item.narrative_teaser = collapse(teaser);
  }
}
...
function normalizeCase(item) {
  ...
  return {
    ...
    narrative_teaser: decodeEntities(item.narrative_teaser || "")
  };
}
